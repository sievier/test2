...
class encodertitle(nn.Module):
    def __init__(self,vocab_size,embedding_size,hidden1,file='WordID_embedding.npy'):  #WordID_embedding
        ...
        self.bilstm=nn.LSTM(input_size=embedding_size,hidden_size=hidden1,num_layers=1,bidirectional=True,batch_first=True)
    ...
    def forward(self,x):
        ...
        #print('title_embedding_output.size:',output.size())# b*23*300
        #print('title_embedding_output:', output)
        output=self.dopout(output)
        output,_=self.bilstm(output,(h0,c0))
        #print('title_embedding_BiLSTM_output.size:', output.size())  #b*23*256
        #print('title_embedding_BiLSTM_output:', output)
        return output
...
class attention_TP_Title(nn.Module):  #'Word level-attention，Title中word的attention计算
    ...
    def forward(self,x,y): #x=userIDEmbedding,y=title
        ...
        #print('Word level-attention and size:',ai, ai.size())#([B, L# , 1])
        ai_mask = binaryMatrix(ai, 1)  # ([B, L# , 1])
        #print ('ai_mask and size:',ai_mask.size(),ai_mask)
        exp_ai=torch.exp(ai)*ai_mask
        #print('exp_ai and fater Mask:',exp_ai, exp_ai.size())
        ai=exp_ai/torch.sum(exp_ai,dim=1)
...
class TPEncoder(nn.Module):
    def __init__(self,cfg):
        ...
        self.encodertile=encodertitle(vocab_size=cfg.vocab2,embedding_size=cfg.embedding_size2,hidden1=cfg.hidden1)
    ...
    def forward(self,x1,x2,x3,x4,x5):
        ...
        title=self.encodertile(x2)
        #print('mask_title.size:', mask_title.size())  #[B, 23, 256]
        mask_title=binaryMatrix(x2,self.hidden1*2)  #title tensor对应的mask
        #print('mask_title:',mask_title)  #b*23*256
        title=torch.mul(title,mask_title)  #title经过bi-lstm处理后的Mask操作
        #print('title after mask.size:', title.size()) #[B, 23, 256]
        #print('title after mask:', title)
        #print('x3,x4,x4:',x3,x4,x4)
        destination=self.ecoderDestination(x3)
        cattion1,cattion2=self.Categories(x4,x5)
        user_title=self.attention_TP_Title(x=user,y=title)  #Word_level Attention
        ...
        #print('destination_embedding',destination)
        #print('cattion1_embedding', cattion1)
        #print('cattion2_embedding',cattion2)
        #print('user_title size:',user_title.size())  #[1, 256]
...
class Biattention_one(nn.Module):  #long_term的表达学习
    ...
    def build_shape(self,input_shape):
        ...
        #self.w = nn.Parameter(0.1 * torch.rand(size=(1, num_steps, input_size)))
        self.b=nn.Parameter(torch.zeros(size=(1,1,input_shape[2])))
    def forward(self,q_u,L,mask_longterm): #U:user;L:long-term-encoder
        #B,N,L]
        ...
        output=torch.mul(output,mask_longterm)
